...
pub const MEMORY_MODE: &str = "memory";
...
// In-memory stat buffer used when STAT_FILE is "memory".
static MEMORY_CONTENT: StdRwLock<Vec<u8>> = StdRwLock::new(Vec::new());

pub fn memory_content() -> String {
    let guard = MEMORY_CONTENT.read().unwrap();
...
pub async fn show_stats(filepath: &str) {
    ...
    if filepath == MEMORY_MODE {
        *MEMORY_CONTENT.write().unwrap() = stats_content.into_bytes();
        info!("Statistics updated in memory");
        return;
    }
...
// Periodic loop: record process memory/uptime metrics and dump statistics.
pub async fn periodic_dump(filepath: &str, interval: Duration) {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_show_stats_memory_mode() {
        let _g = test_guard().await;
        ...
        assert!(content.contains("test_service_7"), "Memory buffer should contain service name");
        assert!(content.contains("traffic:"), "Memory buffer should contain traffic section");
    }
}
