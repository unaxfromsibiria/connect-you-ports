                        ...
                        Ok(n) => {
                            ...
                            match data_handler.load_data_message(&data) {
                                Ok(msg) => {
                                    ...
                                    debug!("Forwarding {} bytes to client channel", client_data.len());
    ...
    loop {
        tokio::select! {
            ...
            n = reader.read(&mut buf) => {
                ...
                if n < 1 {
                    debug!("Closing TCP connection for client {} in {} {}", ip, t_inf, service_name);
                    break;
                }
                out_bytes = n;
                if to_server_channel.send((Uuid::nil(), Bytes::from(buf[..n].to_vec()))).await.is_err() {
                    ...
                    debug!("Forwarded {} bytes to server channel for {} in {}", n, t_inf, service_name);
                }
            },
        ...
        tokio::select! {
            res = socket.recv_from(&mut buf) => {
                ...
                if to_server_channel.send((transfer_id, Bytes::from(buf[..n].to_vec()))).await.is_err() {
                    ...
                    debug!("Sent {} bytes to channel for client {} in service {}", n, transfer, service_name);
                }
            },
...
async fn tcp_client_processing(settings: &Settings, tasks: &mut JoinSet<()>) {
...
async fn udp_client_processing(settings: &Settings, tasks: &mut JoinSet<()>) {
    let data_handler = Arc::new(DataHandlerSettings::new(settings));
